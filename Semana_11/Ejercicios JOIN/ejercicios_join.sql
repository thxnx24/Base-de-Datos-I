...
SELECT DISTINCT e.name FROM Eats e INNER JOIN Person p ON e.name = p.name WHERE p.gender = 'female' AND (e.pizza = 'mushroom' OR e.pizza ='pepperoni');

/*c. Encuentra los nombres de todas las mujeres que comen pizza de champiñones y pepperoni.*/
SELECT DISTINCT p.name FROM Person p INNER JOIN Eats e1 ON p.name = e1.name INNER JOIN Eats e2 ON p.name = e2.name
...
SELECT DISTINCT f.pizzeria FROM Frequents f INNER JOIN Person p ON f.name = p.name WHERE p.gender = 'female' AND f.pizzeria NOT IN (
...
SELECT DISTINCT f.pizzeria FROM Frequents f INNER JOIN Person p ON f.name = p.name WHERE p.gender = 'male' AND f.pizzeria NOT IN (
...
LEFT JOIN (
    SELECT F.name, S.pizza
    FROM Frequents F
    INNER JOIN Serves S ON F.pizzeria = S.pizzeria
) FS ON E.name = FS.name AND E.pizza = FS.pizza
WHERE FS.name IS NULL;


SELECT E.name, E.pizza FROM Eats E LEFT JOIN 
(SELECT F.name, S.pizza FROM Frequents F INNER JOIN Serves S ON F.pizzeria = S.pizzeria) FS ON E.name = FS.name AND E.pizza = FS.pizza WHERE FS.pizza IS NULL;